...
class SaleOrder(models.Model):
    ...
    invoice_ids = fields.Many2many("account.move", string='Invoices', compute="_get_invoiced", copy=False,
    ...
    currency_rate = fields.Float("Currency Rate", compute='_compute_currency_rate', store=True, digits=(12, 6),
    ...
    def _compute_field_value(self, field):
        if field.name == 'invoice_status' and not self.env.context.get('mail_activity_automation_skip'):
            filtered_self = self.filtered(
                lambda so: (so.user_id or so.partner_id.user_id) and so._origin.invoice_status != 'upselling')
    ...
    def _show_cancel_wizard(self):
        for order in self:
            if order.invoice_ids.filtered(lambda inv: inv.state == 'draft') and not order._context.get(
    ...
    def export_data_csv(self):
        ...
        with open('product_list.csv', mode='w') as employee_file:
            ...
            employee_writer.writerow(
                ['Product ID', 'Product Barcode', 'Product internal reference', 'Sales count', 'Average price unit',
                 'Sales price'])
            ...
            for k, array_v in products_id_price.items():
                for record_product in self.env['product.product'].search([('id', '=', k)]):
                    employee_writer.writerow([record_product.id, record_product.barcode, record_product.default_code,
